...
function convertDaysToWeeksAndDays(days: number): { weeks: number, remainingDays: number } {
...
function suggestClothing(temperature: number): string {
    if (temperature < 10) return "Wear heavy winter clothes.";
    if (temperature < 20) return "Wear  a warm clothes.";
...
function checkDivisibility(number: number): string {
    if (number % 3 === 0 && number % 5 === 0) return "Divisible by both 3 and 5.";
